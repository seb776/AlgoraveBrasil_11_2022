...
vec3 getMat(vec3 p, vec3 n, vec3 rd, vec3 res)
{
  ...
  if (res.z == 0.)
  {
    ...
    p2.xz *= r2d(mix(hash(it), hash(float(int(time+1)), mod(time, speed)/speed));
    p2.yz *= r2d(sin(time*.3)*.5);
    vec2 uv = p2.xy;
    ...
    vec3 rgb = vec3(0.);
    rgb = mix(vec3(0.,.4,.2), vec3(0.,0.5,0.8), textureRepeat(greyTex, uvi).x)
    *(pow(textureRepeat(greyTex, uvi*.5).x, 2.)+.5)
    *pow(sat(-dot(rd, n)), 2.);

    col = mix(col, rgb, 1.-sat(border*400.));
vec3 h = normalize(rd+ldir);
    col += vec3(1.)*pow(sat(-dot(h, n)), 45.);
  }
